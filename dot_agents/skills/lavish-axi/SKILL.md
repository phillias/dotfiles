...
- Fleet operational notes (hard-won 2026-09 kalí session, verify before trusting):
  - The published virtual host is `lavish.<fleet dnsDomain>` (chezmoi `$fleet.dnsDomain`, currently `phillias.us`), NOT phillias.org in the older examples. Boot the server with matching `LAVISH_AXI_ALLOWED_HOSTS` and `LAVISH_AXI_LINK_HOST`; a wrong virtual host boots a server that answers `-Host: <wrong>`-requests with 403 `forbidden host`.
  ...
  - Captain-facing URL is the godoxy route `http://lavish.phillias.us/session/<id>` (no port suffix): godoxy route → Docker-label-discovered `lavish-proxy` origin container `:4388` → host server `:4387`. Debug in that order (server health → proxy → route label).
