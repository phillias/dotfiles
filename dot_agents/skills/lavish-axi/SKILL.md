...
- Before running `npx -y lavish-axi <html-file>`, ensure the Lavish Docker proxy is running. Check: `docker ps --filter name=lavish-proxy --filter status=running -q | grep -q .` If running, verify the target is reachable: `docker exec lavish-proxy sh -c "nc -z host.docker.internal 4387" 2>/dev/null` — if that fails, the previous server exited; kill the stale proxy: `docker rm -f lavish-proxy` then start fresh. If not running, start: `docker run -d --name lavish-proxy -p 4388:4388 --add-host=host.docker.internal:host-gateway -e REMOTE_HOST=host.docker.internal -e REMOTE_PORT=4387 -e LOCAL_PORT=4388 --label proxy.lavish.alias=lavish --label proxy.lavish.port=4388 --label proxy.lavish-proxy.exclude=true marcnuri/port-forward`. Godoxy discovers routes dynamically via Docker labels — no config changes or restarts needed. The `proxy.lavish.alias=lavish` label makes godoxy serve it at `lavish.$FLEET_DNS_DOMAIN`.
- Before opening a session, start the Lavish server on the host if not already running: check `curl -sf http://127.0.0.1:4387/health` — if that fails, start it: `LAVISH_AXI_HOST=0.0.0.0 LAVISH_AXI_PORT=4387 LAVISH_AXI_ALLOWED_HOSTS=lavish.${FLEET_DNS_DOMAIN:?unset} LAVISH_AXI_LINK_HOST=lavish.${FLEET_DNS_DOMAIN:?unset} lavish-axi server --port 4387 --host 0.0.0.0 &` The server must bind to `0.0.0.0` (not `127.0.0.1`) so the Docker bridge can reach it. `ALLOWED_HOSTS` is required or the server rejects external requests with "forbidden host". `$FLEET_DNS_DOMAIN` is set in `~/.zshenv` from chezmoi `fleet.dnsDomain`.
...
- After running `npx -y lavish-axi end <html-file>`, stop and remove the Lavish Docker proxy: `docker rm -f lavish-proxy`. The annotated artifact is on the host filesystem in `.lavish/`.
...
- Fleet operational notes (hard-won 2026-09 kalí session, verify before trusting):
  - The published virtual host is `lavish.<fleet dnsDomain>` (chezmoi `$fleet.dnsDomain`, shell `$FLEET_DNS_DOMAIN`, currently `phillias.us`). Boot the server with matching `LAVISH_AXI_ALLOWED_HOSTS=lavish.$FLEET_DNS_DOMAIN` and `LAVISH_AXI_LINK_HOST=lavish.$FLEET_DNS_DOMAIN`; a wrong domain boots a server that answers `Host:`-header requests with 403 `forbidden host`, and browser chat POST requests with 403 `cross-origin prompt submission rejected` (Lavish's `isSameOriginRequest()` rejects the `Origin:` header when the allowed-hostnames set doesn't include the godoxy hostname).
  - The server **idle self-stops** (no connected clients ⇒ process exits). Browsers then see 502/closed-connection errors from the reverse-proxy path. Restart it with a deliberate `setsid -f` spawn during active review; the long-poll cycles keep it alive while a review is open. Do not wrap it in a systemd unit (captain does not want that imposed).
  - Captain-facing URL is the godoxy route `https://lavish.$FLEET_DNS_DOMAIN/session/<id>` (no port suffix): Cloudflare → godoxy route → Docker-label-discovered `lavish-proxy` container `:4388` → host server `:4387`. Debug in that order (server health → proxy → route label).
  - Browser extensions (ad-blockers) kill the CDN `jsdelivr` fetches with `ERR_BLOCKED_BY_CLIENT`, which cascades to broken Mermaid diagram renders (`translate(undefined, NaN)`). The durable fix is **vendoring page assets locally**: download `daisyui.css`, `themes.css`, `tailwind-browser.js`, and Mermaid's bundled UMD `mermaid.min.js` from jsdelivr into the artifact directory, reference them with relative paths, and switch the Mermaid init from the ESM module to the UMD build with `window.mermaid.run`. Vendored artifacts keep working with any blocker and offline.
- Lavish does not auto-inject any design system - artifacts stay portable so they render identically when opened directly without lavish-axi running. Before writing any HTML: Decide the design direction in this strict priority order, and only move to the next step when the current one truly yields nothing: (1) if the user asked for a specific look or named design system, use that; (2) otherwise you must first inspect the project the artifact is about - the subject or product whose content or UI it represents, which may differ from your current working directory - match that project's design system: Tailwind or theme config, shared CSS variables or design tokens, component library, brand assets, or existing styled pages. If the artifact previews, proposes, or mocks a specific app's UI, render it in that app's own design system so it faithfully shows the product, even when you are running in a different repo; (3) only when both steps come up empty, use the Lavish-recommended Tailwind CSS browser runtime v4 + DaisyUI v5, available via CDN, and prefer that CDN snippet over hand-writing styles unless explicitly instructed otherwise by the user. Run `npx -y lavish-axi design` for a content-to-playbook router, a copy-pasteable CDN snippet, a Mermaid CDN snippet/init for diagrams, and the DaisyUI component reference. When you deliver the artifact, state which of the three design sources you used and why.
- Use lavish-axi when the user asks for a visual artifact, HTML explainer, interactive prototype, review surface, product or technical plan, comparison, report, or browser-based feedback loop
