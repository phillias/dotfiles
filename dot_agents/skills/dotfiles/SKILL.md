...
Note: `chezmoi-axi sync` also runs automatically via cron every 30 min, driven by the `chezmoi-sync-cron` wrapper which writes timestamped history to `~/.local/share/chezmoi/.chezmoi-sync.log`.
...
The preferred path (no-mistakes) opens the PR and owns branch cleanup and merge monitoring. Other machines pick up merged changes on next `chezmoi-axi sync` (cron every 30 min via `chezmoi-sync-cron`).
