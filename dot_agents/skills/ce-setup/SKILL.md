...
2. **If approved:** Run the install command using a shell execution tool. After the command completes, verify installation:
   ...
   - For an agent skill, prefer `npx --yes skills list --global --json | jq -r '.[].name' | grep -qx <skill-name>` when `npx` is available; otherwise fall back to checking that `~/.agents/skills/<skill-name>` (canonical) or legacy `~/.config/opencode/skills/<skill-name>` / `~/.codex/skills/<skill-name>` exists (file, directory, or symlink).
