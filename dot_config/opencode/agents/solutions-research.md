...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with `compound` (the compound skill) after it lands — the absence is itself useful signal.
...
- Any planning or design flow that wants institutional grounding before committing to an approach
- Any review or optimization pass that should check whether a past learning already covers the change
